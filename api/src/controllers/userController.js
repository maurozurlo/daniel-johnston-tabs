...
const assertUserData = async (username, password) => {
    const [rows] = await pool.execute('SELECT id, password FROM `users` WHERE `username` = ?', [username]);
    const [user] = rows
    //User not found
...
const userController = {
    register: async (req, res) => {
        ...
        try {
            ...
            await addToDB(tableName, userData, pool)
    ...
    login: async (req, res) => {
        ...
        try {
            const {id} = await assertUserData(username, password)
    ...
    update: async (req, res) => {
        ...
        try {
            await assertUserData(username, password)
            await updateUserData(id, newUsername, newPassword)
    ...
    delete: async (req, res) => {
        ...
        try {
            await deleteFromDB(tableName, id)
