...
import { getChordRootAndType } from './transpose'
...
export const getKeyFromString = (value: string) => {
  const { root } = getChordRootAndType(value)
  console.log(root)
  return keys.find((note) => note.name === root)?.value ?? -1
}
